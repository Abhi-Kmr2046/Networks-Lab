...
int HandleAPPMsgRecv(int socket, msg_t message)
{
...
int HandleACKMsgRecv(int socket, msg_t message)
{
    for (int i = 0; i < R_CONSTANT; i++)
    {
    ...
    for (int i = 0; i < R_CONSTANT; i++)
    {
        if (connection[i].valid)
        {
            for (int j = 0; j < connection[i].send_count; j++)
            {
                ...
                transmit_count++;
                int k = 0;
                while (connection[i].unack_table[k].id != 0)
                    k++;
...
int r_bind(int socket, int port)
{
...
int r_vconnect(int socket, char dst_ip[128], int dst_port)
{
...
r_sendto(int socket, const void *buffer, size_t length)
{
    ...
    {
        i = 0;
        while (connection[socket].unack_table[i].id == 0)
            i++;
    } while (i == R_CONSTANT);
...
int enqueue_recv_buffer(int socket, msg_t message)
{
...
msg_t dequeue_recv_buffer(int socket)
{
...
int full_recv_buffer(int socket)
{
...
int print_message(msg_t message)
{
    printf("id: %d\n", message.id);
    if (message.type == R_APP)
        printf("type: R_APP\n");
