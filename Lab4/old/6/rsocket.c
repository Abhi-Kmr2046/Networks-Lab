...
ssize_t r_sendto(int sockfd, const void *buf, size_t len, int flags,
    ...
    memcpy(unack_msg_table[unack_msg_last + 1].message, buffer,
           len + sizeof(id_count) + 1);
    unack_msg_table[unack_msg_last + 1].msg_len = len + sizeof(id_count) + 1;
    unack_msg_table[unack_msg_last + 1].dest_addr =
    ...
    int nbytes_sent = sendto(sockfd, buffer, len + sizeof(id_count) + 1, flags,
                             dest_addr, addrlen);
...
ssize_t r_recvfrom(int sockfd, void *buf, size_t len, int flags,
    ...
    // Deteremine the size of message to read - need to read either asked size
    // len, or the message length actually present, whichever is minimum
    ...
    while (1) {
        ...
        // If some message is received
            ...
            if (FD_ISSET(sockfd, &readfds)) {
                ...
                len = sizeof(src_addr);
                // Receive message
                int msg_len = recvfrom(sockfd, buffer, 110, 0,
                ...
                if (dropMessage(DROP_PROB) == 0) {
                    ...
                    HandleReceive(sockfd, buffer,
                                  (const struct sockaddr *)&src_addr, msg_len);
                } else {
                    // Else do nothing
...
void HandleRetransmit(int sockfd) {
    ...
    for (i = 0; i <= unack_msg_last; i++) {
        ...
        if (curr_time.tv_sec - unack_msg_table[i].tv.tv_sec >= TIMEOUT_T_SEC) {
            ...
            if (sendto(sockfd, unack_msg_table[i].message,
                       unack_msg_table[i].msg_len, 0,
                       (const struct sockaddr *)&unack_msg_table[i].dest_addr,
                       sizeof(unack_msg_table[i].dest_addr)) < 0) {
                perror("Unable to send");
                exit(1);
            }
...
void HandleReceive(int sockfd, char *buffer, const struct sockaddr *src_addr,
                   int msg_len) {
    // If the message is an application message
    if (buffer[0] == 'M') {
        HandleAppMsgRecv(sockfd, buffer, (const struct sockaddr *)src_addr,
                         msg_len);
    }
...
void HandleACKMsgRecv(char *buffer) {
    ...
    for (i = 0; i <= unack_msg_last; i++) {
        if (unack_msg_table[i].id == id) {
            ...
            // Decrement the last message, effectively deleting the acknowledged
            // message
            unack_msg_last--;
            break;
        }
...
void HandleAppMsgRecv(int sockfd, char *buffer, const struct sockaddr *src_addr,
                      int msg_len) {
    int i;
    int id;
...
void sendAck(int id, int sockfd, const struct sockaddr *dest_addr) {
    ...
    if (sendto(sockfd, buffer, sizeof(id_htonl) + 1, 0, dest_addr,
               sizeof(*dest_addr)) < 0) {
        perror("Unable to send");
        exit(1);
    }
